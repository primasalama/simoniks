...
class Kebijakan extends CI_Controller {
	...
	public function add()
	{
		...
		$data = array('narasi' => $this->input->post('narasi'),'status'=>$this->input->post('status'),'indikator'=>$this->input->post('indikator'),'pic'=>$this->input->post('pic'),'created_by'=>$created_by,'updated_by'=>$this->session->userdata('session')[0]->no,'tahun'=>date("Y"),'id_kabid'=>$this->input->post('id_kabid'));
		// print_r($data);die();
