...
class Fagenda extends CI_Controller {
	...
	public function add()
	{
		...
		$data = array('narasiKebijakan'=>$this->input->post('narasiKebijakan'),'uraian'=>nl2br($this->input->post('uraian')),'tanggal'=>$this->input->post('tanggal'),'hasil'=>nl2br($this->input->post('hasil')),'created_by'=>$this->session->userdata('session')[0]->no,'updated_by'=>$this->session->userdata('session')[0]->no);
